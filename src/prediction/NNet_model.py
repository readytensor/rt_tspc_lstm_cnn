...
from torch.nn import Flatten, Softmax, Linear, Module, CrossEntropyLoss, Dropout, LSTM, Conv1d
...
class Net(Module):
    ...
    def __init__(self, feat_dim, encode_len, n_classes, activation, lr):
        ...
        self.output = Linear(
            in_features=self.conv2.out_channels * self.encode_len,
    ...
    def forward(self, X):
        batch_size = X.size(0)
        h0_1 = torch.zeros(
            1, batch_size, self.lstm1.hidden_size).to(self.device)
        c0_1 = torch.zeros(
            1, batch_size, self.lstm1.hidden_size).to(self.device)
